...
export default function OrderBook() {
  ...
  useEffect(() => {
    ...
    socket.onopen = () => {
      socket.send(
        JSON.stringify({
          op: 'subscribe',
          args: ['update:BTCPFC', 'tradeHistoryApi:BTCPFC'],
        })
      );
    };
    ...
    socket.onmessage = (event) => {
      ...
      if (message.topic?.startsWith('update:BTCPFC') && message.data) {
        console.log('data from update', message.data)
        const newBids = (message.data.bids || []).slice(0, 8).map(([price, size]) => ({
          price: parseFloat(price),
          size: parseFloat(size),
        }));

        const newAsks = (message.data.asks || []).slice(0, 8).map(([price, size]) => ({
          price: parseFloat(price),
          size: parseFloat(size),
        }));
        ...
        const getHighlightFlags = (newOrders, oldOrders) =>
          newOrders.map((order, i) =>
            order.price !== oldOrders[i]?.price || order.size !== oldOrders[i]?.size
          );

        const getCellHighlights = (newOrders, oldOrders) =>
          newOrders.map((order, i) => {
            const old = oldOrders[i] || { size: 0 };
        ...
        if (
          ...
          if (!throttleRef.current) {
            ...
            setTimeout(() => {
              ...
              setHighlightRows({ bids: Array(8).fill(false), asks: Array(8).fill(false) });
              setHighlightCells({ bids: Array(8).fill(null), asks: Array(8).fill(null) });
            }, 200);
          }
        }
      }

      if (message.topic === 'tradeHistoryApi:BTCPFC' && Array.isArray(message.data)) {
        console.log('data from tradeHistoryApi', message.data)
  ...
  const renderLastPrice = () => {
    ...
    return (
      <div style={{ textAlign: 'center', padding: '6px 0', fontWeight: 'bold', color, backgroundColor: bg }}>
        {lastPrice ? `${lastPrice.toLocaleString()} ${arrow}` : '-'}
      </div>
    );
  };

  const renderCellBackground = (type, side, value) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 4px', color: '#8698aa', fontSize: 12 }}>
        <div>Price (USD)</div>
      ...
      {asks.map((ask, i) => {
        const cumulative = asks.slice(0, i + 1).reduce((sum, item) => sum + item.size, 0);
        const ratio = totalAskSize > 0 ? cumulative / totalAskSize : 0;
        return (
          <div
            key={`ask-${i}`}
            style={{
              ...rowStyle(highlightRows.asks[i]),
              color: ask.price > 0 ? '#FF5B5A' : '#444',
            }}
          >
            <div style={{ flex: 1, textAlign: 'right', zIndex: 1 }}>{ask.price ? ask.price.toLocaleString() : '-'}</div>
            <div style={{ flex: 1, textAlign: 'right', zIndex: 1, backgroundColor: renderCellBackground(highlightCells.asks[i], 'ask', ask.size), color: '#F0F4F8' }}>{ask.size ? ask.size.toLocaleString() : '-'}</div>
            <div style={{ flex: 1, textAlign: 'right', zIndex: 1, color: '#F0F4F8' }}>{cumulative ? Math.round(cumulative).toLocaleString() : '-'}</div>
            {ask.price > 0 && (
              <div style={{
                position: 'absolute',
                top: 0,
      ...
      {bids.map((bid, i) => {
        const cumulative = bids.slice(0, i + 1).reduce((sum, item) => sum + item.size, 0);
        const ratio = totalBidSize > 0 ? cumulative / totalBidSize : 0;
        return (
          <div
            key={`bid-${i}`}
            style={{
              ...rowStyle(highlightRows.bids[i]),
              color: bid.price > 0 ? '#00b15d' : '#444',
            }}
          >
            <div style={{ flex: 1, textAlign: 'right', zIndex: 1 }}>{bid.price ? bid.price.toLocaleString() : '-'}</div>
            <div style={{ flex: 1, textAlign: 'right', zIndex: 1, backgroundColor: renderCellBackground(highlightCells.bids[i], 'bid', bid.size), color: '#F0F4F8' }}>{bid.size ? bid.size.toLocaleString() : '-'}</div>
            <div style={{ flex: 1, textAlign: 'right', zIndex: 1, color: '#F0F4F8' }}>{cumulative ? Math.round(cumulative).toLocaleString() : '-'}</div>
            {bid.price > 0 && (
              <div style={{
                position: 'absolute',
